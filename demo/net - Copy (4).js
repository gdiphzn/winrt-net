		...
		execute: function () {
			...
			_export('Socket', Socket = function (_Duplex) {
				...
				_createClass(Socket, [{
					...
					value: function _readChunk() {
						...
						//console.log('this.readable', this.readable)
						//console.log('this._readableState.reading', this._readableState.reading)
						//console.log('this._readableState.endEmitted', this._readableState.endEmitted)
						//console.log('################################# _readChunk ####################################')
					...
					value: function _onRead(chunkBytesRead) {
						...
						console.log('_onRead', chunkBytesRead);
						if (chunkBytesRead == 0) {
					...
					value: function _destroy(exception, cb) {
						...
						var fireErrorCallbacks = function fireErrorCallbacks() {
							...
							if (exception && !_this6._writableState.errorEmitted) {
								process.nextTick(emitErrorNT, _this6, new Error(exception));
						...
						setImmediate(function () {
							if (_this6._waitForEnd && !_this6._readableState.endEmitted) {
								// If there is an 'end' event, 'close' cannot preceed it.
								// In end() before calling _destroy() we pushed null to make the stream
								// emit 'end', but that hasn't happened yet
								_this6.once('end', function () {
									return _this6.emit('close', _this6._hadError || !!exception);
								});
								_this6._waitForEnd = false;
							} else {
								// destroy does not call 'end', on 'close'
								_this6.emit('close', _this6._hadError || !!exception);
							}
						});
      	...
      	this._handle = null;
					...
					value: function _write(chunk, encoding, done) {
						...
						//console.log('_write', chunk.length, chunk.toString())
						...
						if (this.writable) {
							...
							this._writer.storeAsync().done(function () {
								_this8._bytesDispatched += chunk.length;
								done();
							}, function (err) {
      	...
      	this._handle.setKeepAlive(setting, ~~(msecs / 1000));*/
			...
			_export('Server', Server = function (_EventEmitter) {
				...
				function Server(options, connectionListener) {
					...
					var _this9 = _possibleConstructorReturn(this, Object.getPrototypeOf(Server).call(this));

					_this9._host = null;
					_this9._connections = 0;


					if (!(_this9 instanceof Server)) return _ret2 = new Server(options, connectionListener), _possibleConstructorReturn(_this9, _ret2);

					_this9._onError = _this9._onError.bind(_this9);
					_this9._onListen = _this9._onListen.bind(_this9);
					_this9._onConnection = _this9._onConnection.bind(_this9);

					EventEmitter.call(_this9);

					if (typeof options === 'function') {
						connectionListener = options;
						options = {};
						_this9.on('connection', connectionListener);
					} else {
						options = options || {};
					...
					_this9.allowHalfOpen = options.allowHalfOpen || false;
					_this9.pauseOnConnect = !!options.pauseOnConnect;
				...
				_createClass(Server, [{
					...
					value: function listen(port, cb) {
						...
						this._handle.bindServiceNameAsync(port).done(this._onListen, function (err) {
							return _this10._onError(err, 'listen', _this10._host, port);
						});
						return this;
					}
				}, {
			...
			_export('isIPv4', isIPv4 = function isIPv4(ip) {
				return IPv4Regex.test(ip);
			});

			_export('isIPv4', isIPv4);

			_export('isIPv6', isIPv6 = function isIPv6(ip) {
				return IPv6Regex.test(ip);
			});

			_export('isIPv6', isIPv6);

			_export('isIP', isIP = function isIP(ip) {
