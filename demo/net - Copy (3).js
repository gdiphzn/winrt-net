...
System.register(['events', 'buffer', 'stream'], function (_export, _context) {
	var EventEmitter, Buffer, Duplex, _typeof, _createClass, process, DataWriter, DataReader, StreamSocketListener, StreamSocket, Socket, Server, connect, createConnection, errorCodes;
			...
			_export('Socket', Socket = function (_Duplex) {
				...
				_createClass(Socket, [{
					...
					value: function _onConnect() {
						...
						console.log('_onConnect this._connected', this._connected);
					...
					value: function _onReceive(chunkBytesRead) {
						...
						console.log('_onReceive', chunkBytesRead);
						console.log('this._connected', this._connected);
						if (this.destroyed) {
						...
						if (chunkBytesRead == 0) {
							console.log('received 0 == server closed this socket, calling end()');
							this.end();
							return;
						}
					...
					value: function _onError(err, syscall, address, port) {
						...
						if (this._closing || this.destroyed) {
							console.log('error but already closing');
					...
					value: function end(data, encoding) {
						...
						// WARNING: Duplex.end() call this.destroy
						//console.log('BEFORE DUPLEX.END()')
						//Duplex.prototype.end.call(this, data, encoding)
						//console.log('AFTER DUPLEX.END()')
						if (data) {
							Duplex.prototype.write.call(this, data, encoding);
						}
						//if (!this._connected) {
						...
						console.log('end() b');
						if (this._closing || this.destroyed) {
							// redundant call, already closing or destroying
							return;
						}
						console.log('end() c');
			...
			_export('Server', Server = function (_EventEmitter) {
				...
				_createClass(Server, [{
					...
					value: function getConnections(cb) {
						setImmediate(cb, null, this._connections);
					}
				}, {
					key: '_onError',
					value: function _onError(err, syscall, address, port) {
